...
def complete_theme(datasets_theme, topics_list, llm_response_format):
  for name, desc in datasets[datasets_theme].items():
...
"""
      ...
      response = get_definite_response(messages, llm_response_format)
      objresponse = None
      if response != None:
      ...
      if response != None and validate_response(objresponse):
        for topic in topics_list:
          if topic in objresponse:
            datasets[datasets_theme][name][topic].append({
                  "items" : objresponse[topic],
                  "source" : source,
...
try:
  ...
  complete_theme("definitions", definition_topics, definition_text_extraction_response_format)
  print("Completing theorems in datasets...")
  complete_theme("theorems", theorem_topics, theorem_text_extraction_response_format)
  print("Done")
